...
class Middle(ABC):
    ...
    @abstractmethod
    def get_average(self, users=True, sorting=False):
        ...
        return sorted(votes) if sorting else votes
...
class Average(Middle):
    def get_average(self, users=True):
        """Возвращает среднее арифметическое пользовательских оценок или
        оценок критиков в зависимости от значения параметра users"""
        votes = super().get_average(users=users)
...
class Median(Middle):
    def get_average(self, users=True):
        ...
        оценок критиков в зависимости от значения параметра users"""
        votes = super().get_average(users=users, sorting=True)
